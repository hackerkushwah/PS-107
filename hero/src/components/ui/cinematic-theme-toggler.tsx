...
export function CinematicThemeToggler({
  ...
  // ── Click handler: particle burst + View Transition ──────────────────────
  const handleClick = () => {
    setBurst(true);
  ...
  return (
    <button
      ...
      className={cn(
        "relative h-7 w-14 rounded-full border overflow-hidden cursor-pointer select-none",
    ...
    >
        ...
        {showFill && coverSize > 0 && (
          <motion.span
            ...
            className={cn(
              ...
              isDark ? "bg-indigo-500/60 shadow-[0_0_16px_rgba(99,102,241,0.5)]" : "bg-amber-200/80"
            )}
            style={{
              width: coverSize,
              height: coverSize,
          ...
          particles.map((p) => (
            <motion.span
              ...
              className={cn(
                ...
                isDark ? "bg-blue-300" : "bg-amber-500"
              )}
      ...
      <span
        ...
        className={cn(
          ...
          isDark
            ? "bg-gradient-to-r from-slate-900 via-blue-950/60 to-slate-800"
            : "bg-gradient-to-r from-amber-50 via-yellow-50 to-orange-50"
        )}
      />

      {/* Decorative stars / rays */}
      <span aria-hidden className="absolute inset-0 pointer-events-none">
        {isDark ? (
          <>
            <span className="absolute top-[28%] left-[16%] w-0.5 h-0.5 rounded-full bg-white opacity-70" />
            <span className="absolute top-[62%] left-[26%] w-0.5 h-0.5 rounded-full bg-blue-200 opacity-60" />
            <span className="absolute top-[44%] left-[10%] w-1 h-1 rounded-full bg-blue-300 opacity-50" />
          </>
        ) : (
          <>
            <span className="absolute top-[30%] right-[18%] w-0.5 h-0.5 rounded-full bg-amber-600 opacity-60" />
            <span className="absolute top-[62%] right-[26%] w-0.5 h-0.5 rounded-full bg-orange-400 opacity-50" />
          </>
        )}
      </span>
      ...
      <motion.span
        ...
        className={cn(
          "absolute top-1/2 z-10 flex items-center justify-center rounded-full shadow-md",
          "-translate-y-1/2 w-5 h-5 transition-colors duration-300",
          isDark ? "bg-slate-100 text-slate-800" : "bg-amber-500 text-white"
        )}
      >
